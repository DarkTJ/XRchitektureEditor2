        ...
        if (objectToSample.Length != 0)
        {
            for (int i = 0; i < objectToSample.Length; i++)
            {
                ...
                else if (objectToSample[i].GetComponentInParent<CylLight>() != null)
                {
                    ...
                    color = color + 5*(objectToSample[i].GetComponent<Renderer>().material.GetColor("_EmissionColor"));
                }
